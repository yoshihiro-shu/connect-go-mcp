...
// NewUserServiceMCPServerWithClient はconnectクライアントを使用してMCPサーバーを作成します
func NewUserServiceMCPServerWithClient(client userv1connect.UserServiceClient) *server.MCPServer {
	server := server.NewMCPServer("UserService", "1.0.0")

	connectHandler := connectgomcp.NewConnectHandler(client)
	server.AddTool(
		mcp.NewTool("GetUser ユーザー情報取得",
			mcp.WithDescription("ユーザー情報取得リクエスト パラメータ user_id: ユーザーID（必須） 戻り値: ユーザー名、メールアドレス、作成日時"),
			mcp.WithString("user_id"),
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			return connectHandler.Handle(ctx, req, "GetUser")
		},
	)
	...
	server.AddTool(
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			return connectHandler.Handle(ctx, req, "CreateUser")
		},
	)
	...
	server.AddTool(
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			return connectHandler.Handle(ctx, req, "ListUsers")
		},
	)
